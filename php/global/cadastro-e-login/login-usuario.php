...
$stmt_admin = $conn->prepare("SELECT nome, id_matricula, nivel_acesso, senha_hash FROM adm WHERE id_matricula = ?");
$stmt_admin->bind_param("s", $mat);
$stmt_admin->execute();
...
if($result_admin && $result_admin->num_rows > 0){
    ...
    if(password_verify($senha, $dados_admin["senha_hash"])){
...
$stmt_corretores = $conn->prepare("SELECT nome, id_matricula, email, cpf, nivel_acesso, senha_hash FROM corretores WHERE id_matricula = ?");
$stmt_corretores->bind_param("s", $mat);
...
if($result_corretores && $result_corretores->num_rows > 0){
    ...
    if(password_verify($senha, $dados_corretores["senha_hash"])){
...
$stmt_alunos = $conn->prepare("SELECT nome, id_matricula, email, cpf, turno, turma, idioma, nivel_acesso, senha_hash FROM alunos WHERE id_matricula = ?");
$stmt_alunos->bind_param("s", $mat);
$stmt_alunos->execute();
...
if($result_alunos && $result_alunos->num_rows > 0){
    ...
    if(password_verify($senha, $dados_aluno["senha_hash"])){
